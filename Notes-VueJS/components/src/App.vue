<template>
  <div id="app">
    <toolbar v-on:clickNew="createNote" v-on:clickDelete="deleteNote" v-bind:searchNoteText="searchNoteText" v-on:inputSearchNoteText="updateSearch"></toolbar>
    <note-container v-bind:notes="notes" v-bind:transformedNotes="transformedNotes" v-bind:selectedNote="selectedNote" v-on:selectNote="selectNote" v-on:inputNoteEditor="updateSelectedNote"></note-container>
  </div>
</template>
  ...
  methods: {
    ...
    deleteNote: function () {
      ...
      if (index !== -1) {
        ...
        if (this.transformedNotes.lenght > 0) {
